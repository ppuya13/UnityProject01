...
namespace Monster
{
    public class MonsterController : SerializedMonoBehaviour
    {
        ...
        public InfoPanel connectedPanel;
        ...
        public const float HitFeedbackDuration = 0.05f; //피격 시 시간이 느려지고 몬스터가 떨리는 시간
        public const float MonsterShakeMagnitude = 0.1f; // 몬스터가 떨리는 강도 
        ...
        public void HitCheck()
        {
            ...
            if (attackConfig.RangeType is MeleeAttack config)
            {
                ...
                IColliderConfig hitColliderConfig = default;
                ...
                foreach (var colliderConfig in config.ColliderConfigs)
                {
                    ...
                    switch (colliderConfig.ColliderType)
                    {
                        case ColliderType.Sphere:
                            ...
                            hitColliderConfig = colliderConfig;
                            break;
                ...
                foreach (var hitCollider in allHitColliders)
                {
                    ...
                    if (player)
                    {
                        Vector3 hitPoint = CalculateHitPointWithinAttackRange(hitCollider, basePosition, hitColliderConfig);
                        
                        // 데미지 적용
                        player.AttackValidation(attackConfig, currentAttack, attackIdx, transform, currentHitSound, hitPoint);
                    }
                }
            }
            else
        ...
        public void HitCheckIdx(int idx)
        {
            ...
            if (attackConfig.RangeType is MeleeAttack config)
            {
                ...
                foreach (var hitCollider in hitColliders)
                {
                    ...
                    if (player)
                    {
                        Vector3 hitPoint = CalculateHitPointWithinAttackRange(hitCollider, basePosition, colliderConfig);
                        player.AttackValidation(attackConfig, currentAttack, attackIdx, transform, currentHitSound, hitPoint);
                    }
                }
            }
        ...
        private Vector3 CalculateHitPointWithinAttackRange(Collider hitCollider, Vector3 attackCenter, IColliderConfig colliderConfig)
        {
        ...
        //플레이어의 공격은 서버를 통한다.
        public void AttackValidation(PlayerAttackConfig attackConfig, Transform player, SoundType hitSound, Vector3 hitPoint)
        {
            if (isDie) return; //사망한상태면 넘어감
            ...
            hitFeedbackCoroutine = StartCoroutine(ApplyHitStop()); //게임을 잠시 느리게한다.
            ...
            if (isCounter)
            {
                isCounter = false;
                //카운터중이면 
                animator.SetInteger(CounterIndex, 1); //카운터 성공
                SoundManager.Instance.PlayRandomSound(characterSounds.GetSounds(SoundType.Guard), volume: 0.5f, position: transform.position);
                return;
            }
            ...
            //랜덤한 피격 파티클 생성
            if (attackConfig.HitEffects != null && attackConfig.HitEffects.Length > 0)
            {
                // 랜덤한 이펙트를 선택
                var effect = attackConfig.HitEffects[Random.Range(0, attackConfig.HitEffects.Length)];
        ...
        public void OtherPlayerAttackValidation(PlayerAttackConfig attackConfig, Transform player, SoundType hitSound, Vector3 hitPoint)
        {
            if (hitDict.TryGetValue((attackConfig, player), out bool value))
            {
                //value값은 의미없고, 일단 true면 같은 공격에 맞았다는 뜻
        ...
        public void TakeDamage(float damage, float msgHp, SoundType soundType)
        {
            ...
            // UIManager.Instance.AddLogChat($"몬스터가 데미지를 {damage} 받음. 체력: {CurrentHp} -> {msgHp}, 최대체력: {maxHp}");
        ...
        private void MonsterDie()
        {
            isDie = true;
            agent.ResetPath();
            if (SuperManager.Instance.isHost) SendChangeState(MonsterState.MonsterStatusDie);
        ...
        IEnumerator ApplyHitStop()
        {
            ...
            yield return new WaitForSecondsRealtime(HitFeedbackDuration); // 실제 시간 기준으로 대기
